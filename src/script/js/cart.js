! function() {
    ...
    function goodslist() {
        ...
        $.ajax({
            url: 'http://10.31.163.86/projectname/php/index/beibeidata.php', //获取所有的接口数据
            dataType: 'json'
        }).done(function(data) {
            ...
            $.each(arrsid, function(index, value) {
                ...
                $.each(data, function(index1, value1) {
                    ...
                    if (value == value1.picid) { //遍历判断sid和传入的sid是否相同，方便将那条数据设置到渲染的商品列表中。
                        // console.log($('.goods-item:hidden'))
                        var $clonebox = $('.goods-item:hidden').clone(true, true);
                        // console.log($clonebox)
                        $clonebox.find('.goods-pic').find('img').attr('src', value1.url);
                        $clonebox.find('.goods-pic').find('img').attr('sid', value1.picid);
                        $clonebox.find('.goods-d-info').find('a').html(value1.titile);
                        $clonebox.find('.b-price').find('strong').html(value1.price);
                        $clonebox.find('.b-price').find('b').html(value1.prices);
                        $clonebox.find('.quantity-form').find('input').val(arrnum[index]);
                        //计算每个商品的价格。
                        $clonebox.find('.b-sum').find('strong').html((value1.price * arrnum[index]).toFixed(2));
