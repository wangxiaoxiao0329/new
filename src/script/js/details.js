! function() {
    ...
    //2.将当前的id传给后端获取对应的数据
    $.ajax({
        url: 'http://10.31.163.86/projectname/php/index/details.php',
        data: {
            sid: picid
        },
        dataType: 'json'
        ...
        $('.loadpcp-s').html(data.prices);
        $('.del-price').html(data.delete);
        $('.save').html(data.delete % data.price);
        $('.price-integer').html(data.price);
        $('.price-little').html(data.prices);
    ...
    $('.p-btn a').on('click', function() { //点击加入购物车按钮。
        ...
        //获取当前的按钮对应的商品的sid
        var $sid = $(this).parents('.detail-mate').find('#smallpic').attr('sid');
...
! function() {
    ...
    var bili = $('#spic').width() / $('#bpic').width();
    $('#spic').hover(function() {
        $('#sf').css('visibility', 'visible');
        $('#bf').css('visibility', 'visible');
        $(this).on('mousemove', function(ev) {
            var $left = ev.pageX - $('.detail-wrap').offset().left - $('#sf').width() / 2;
            var $top = ev.pageY - $('.detail-wrap').offset().top - $('#sf').height() / 2;
            if ($left < 0) {
                $left = 0;
